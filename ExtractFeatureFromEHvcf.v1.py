...
@author: Bharati Jadhav <bharati.jadhav@mssm.edu>
...
def parse_vcf(filename):
    ...
    #vcf_data['SampleId']= vcf_data.columns[-1]
    vcf_data['SampleId']=sample_id
    vcf_data=vcf_data.rename(columns={vcf_data.columns[-2]: 'DATA'})
    ...
    vcf_data=vcf_data.rename(columns={'END':'end','VARID': 'LocusId'})
...
def get_features(df):
    ...
    df['A1'] = df['A1'].astype(str).astype(int)
    df['A2'] = df['A2'].astype(str).astype(int)
    df['IRR_A1'] = df['IRR_A1'].astype(str).astype(int)
    df['IRR_A2'] = df['IRR_A2'].astype(str).astype(int)
    df['SPR_A1'] = df['SPR_A1'].astype(str).astype(int)
    df['SPR_A2'] = df['SPR_A2'].astype(str).astype(int)
    df['FR_A1'] = df['FR_A1'].astype(str).astype(int)
    df['FR_A2'] = df['FR_A2'].astype(str).astype(int)
    ...
    df['Total'] = df['IRR_Total'] + df['SPR_Total'] + df['FR_Total']
    df['IRR_Ratio'] = (df['IRR_A1'] / df['IRR_A2']).astype(float)
    df['FR_Ratio'] = (df['FR_A1'] / df['FR_A2']).astype(float)
    
    df['IRR_Ratio'] = df['IRR_Ratio'].replace(np.nan, 0)
    df['IRR_Ratio'] = df['IRR_Ratio'].replace(np.inf, 1)
    return(df)
